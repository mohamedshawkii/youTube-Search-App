...
function SideBar({buttonInput,SetButtonInput,SetToggleSideBar,toggleSideBar}) {
  const Categories = [
    {
      name: "News",
      icon: <FaRegNewspaper />,
  ...
  return (
    <aside>
      <div className={`h-svh fixed top-[88px] left-0 
        ...
        ${toggleSideBar ? 'items-start px-8 py-6': 'items-center px-4 py-3' }`}>
          {Categories.map((item, index) => (
            <button
              ...
              ${toggleSideBar? 'justify-start px-4 py-6' : 'justify-center w-full h-full px-6 py-6 gap-0'}`}
            >
              <div className="text-3xl">{item.icon}</div>
              {toggleSideBar&&(<div className="text-lg font-semibold">{item.name}</div>)}
            </button>
          ))}
        </div>
      </div>
